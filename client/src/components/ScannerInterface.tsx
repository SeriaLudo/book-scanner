...
function ScannerInterface() {
  ...
  async function updateItemInventory(
    bookId: string,
    values: {condition?: BookCondition; format?: BookFormat}
  ) {
    try {
      await updateBook({id: bookId, ...values});
    } catch (error) {
      setStatus(`Failed to update book: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
      ...
      <main className="w-full px-2 py-2 sm:py-4 grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-6 print:block max-w-4xl">
        ...
        <section className="print:hidden">
          <BookList
            ...
            onUpdateItem={updateItemInventory}
